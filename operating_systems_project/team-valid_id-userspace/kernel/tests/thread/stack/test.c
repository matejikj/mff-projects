...
static void* layer_two(void* storage) {
    ...
    err = thread_create(&my_info[1].thread, &layer_three, my_info + 1, 0, my_info[1].name);
    ktest_assert_errno(err, "thread_create(layer_three, first)");

    err = thread_create(&my_info[2].thread, &layer_three, my_info + 2, 0, my_info[2].name);
    ktest_assert_errno(err, "thread_create(layer_three, second)");
...
static void* layer_one(void* storage) {
    ...
    err = thread_create(&my_info[1].thread, &layer_two, my_info + 1, 0, my_info[1].name);
    ktest_assert_errno(err, "thread_create(layer_two, first)");

    err = thread_create(&my_info[4].thread, &layer_two, my_info + 4, 0, my_info[4].name);
    ktest_assert_errno(err, "thread_create(layer_two, second)");
...
void kernel_test(void) {
    ...
    err = thread_create(&infos[0].thread, &layer_one, infos + 0, 0, infos[0].name);
    ktest_assert_errno(err, "thread_create(layer_one, first)");

    err = thread_create(&infos[7].thread, &layer_one, infos + 7, 0, infos[7].name);
    ktest_assert_errno(err, "thread_create(layer_one, second)");
    ...
    for (int i = 0; i < THREAD_COUNT; i++) {
        err = thread_join(infos[i].thread, NULL);
