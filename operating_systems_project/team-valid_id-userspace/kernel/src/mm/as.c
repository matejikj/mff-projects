...
as_t* as_create(size_t size, unsigned int flags) {
    ...
    printk("ALLOCATED FRAME %x OF SIZE %d\n", as_start, size);

    if (result != EOK) {
        panic("Failed to allocated frames for new address space.");
    } 

    as_t* as = kmalloc(sizeof(as_t));

    if (as == NULL) {
        panic("Failed to allocate heap for new address space.");
    }
